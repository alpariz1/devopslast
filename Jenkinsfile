...
pipeline {
    ...
    stages {
        ...
        stage('Building JAR') {
            steps {
                echo 'Start building JAR file'
                bat 'gradle clean bootJar' // Use bat instead of sh
            }
        }
        stage('Create the Docker image') {
            steps {
                echo 'Image has been built' // Missing parenthesis
                bat 'docker build -t alpariz/app .' // Use bat instead of sh
            }
        }
        stage('Login to DockerHub') {
            steps {
                // Pipe DockerHub password securely
                bat "echo %DOCKERHUB_CREDENTIALS_PSW% | docker login -u %DOCKERHUB_CREDENTIALS_USR% --password-stdin" // Corrected syntax
                echo 'Logged in'
            }
        }
        stage('Push the image to DockerHub') {
            steps {
                bat 'docker push alpariz/app123' // Use bat instead of sh
                echo 'Image is pushed' // Missing parenthesis
            }
        }
    }
